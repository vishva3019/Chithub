...
def start_room(data):
    ...
    g = ChitGroup.query.get(int(data['group_id']))
    if g:
        g.is_active, g.highest_bid, g.highest_bidder_name = True, 0.0, "No bids placed yet"
        db.session.commit(); LIVE_BID_LOGS[str(g.id)] = []
        emit('room_opened', {'group_id': g.id, 'highest_bid': 0.0, 'highest_bidder': "No bids placed yet"}, to=str(g.id))

@socketio.on('place_incremental_bid')
def handle_inc_bid(data):
    g = ChitGroup.query.get(int(data.get('group_id')))
    if not g or not g.is_active: return
    ...
    g.highest_bid += increment; g.highest_bidder_name = bidder; db.session.commit()
    ...
    if str(g.id) not in LIVE_BID_LOGS: LIVE_BID_LOGS[str(g.id)] = []
    LIVE_BID_LOGS[str(g.id)].append(log_payload)
    emit('bid_broadcast', {'group_id': g.id, 'highest_bid': g.highest_bid, 'highest_bidder': g.highest_bidder_name}, to=str(g.id))
    emit('single_log_append', log_payload, to=str(g.id))
